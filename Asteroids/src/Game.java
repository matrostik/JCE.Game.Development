...
public class Game extends Applet implements Runnable, KeyListener 
{
	...
	// GAME OVER!!!!
	boolean gameOver, isGameOverPlayed;
	...
	public void paint(Graphics gfx) 
	{
		...
		g2d.drawString("Level : " + level, 10, 15);
		g2d.drawString("Lives : " + lives, 70, 15);
		g2d.drawString("Score : " + score, 130, 15);

		if(isImmortal && !gameOver) 
		{
			g2d.setColor(Color.RED);
			g2d.drawString("Press FIRE to start the game", 360, 375);
		}
		...
		if(gameOver) 
		{
			...
			if(!isGameOverPlayed) 
			{
				System.out.println(isGameOverPlayed);
				Sound.playGameOver();
				//isGameOverPlayed = true;
			}
		}
		gfx.drawImage(img, 0, 0, this); // draw the off-screen image onto the applet
	}
	...
	public void run() 
	{
		...
		for( ; ; ) 
		{
			...
			for(AsteroidExplosion e : explodingLines) 
			{// asteroid explosion floating lines movement
				e.move();
			}
			for(int i = 0 ; i<explodingLines.size(); i++) 
			{// asteroid explosion floating lines removal
				if(explodingLines.get(i).getLifeLeft() <= 0)
					explodingLines.remove(i);
			}
			for(ShipExplosion ex : shipExplosion)
			{// ship explosion expansion
				ex.expand();
			}
	...
	public void addAsteroids() 
	{
		...
		{   // add asteroids to game
			...
			// if there are more then four asteroids, any new ones are big asteroids
			...
			// if asteroid center is within 80 pixels of ship's center, 
			// remove it from the ArrayList and rebuild it
